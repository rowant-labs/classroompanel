...
Copy `.env.example` to `.env.local` and add at least one model API key for live generation — or use the in-app "Bring your own key" panel. Without any key, `/panel` opens with an empty board (the fixed sample board is at `/demo`).
...
The pull-request template includes a checkbox confirming the CLA — ticking it on your PR is the agreement. If you're contributing on behalf of an employer, make sure you're authorized.
...
ClassroomPanel has a solo maintainer working with AI-assisted review — CI (typecheck, tests, build) is the first gate, and every PR gets a substantive read, but response times vary. What speeds a merge up: green CI, a focused diff, a sentence explaining the *why*, and tests for anything behavioral. What slows it down: drive-by refactors, pedagogy changes without grounding, and anything that conflicts with [docs/VISION.md](docs/VISION.md).

## Reporting issues

- **Bugs:** use the issue template — include the route (`/panel`, `/api/lesson`, etc.), the model/provider in use, and — for generation bugs — the lesson JSON if you can capture it. Anything that loses or corrupts a learner record gets priority.
- **Security issues:** do not open a public issue — [report privately](https://github.com/rowant-labs/classroompanel/security/advisories/new) (see [SECURITY.md](SECURITY.md)). This product is used by learners; treat reports accordingly.
